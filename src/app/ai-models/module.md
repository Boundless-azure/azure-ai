...
- MiniMax provider 支持 OpenAI / Anthropic 兼容接口，默认国内 endpoint 为 `https://api.minimaxi.com/v1` 或 `https://api.minimaxi.com/anthropic`，前端可选择国内/海外预设。
...
- AiModelsService.resolveOpenAIBaseURL -> openai-compatible-base-url (含 MiniMax 国内默认 OpenAI 兼容 endpoint)
