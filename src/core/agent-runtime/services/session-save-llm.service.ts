 ...
 * @description 主对话 LLM 完成 call_hook 后, 由 SessionCallTracker 硬匹配命中即异步触发本 service. 用同 agent model 跑一次独立 LLM session, 给它 call_hook 记录 + 现有 sessionData listing, 让它决定是否调 saas.app.conversation.sessionData.save (0~N 次). 主对话 LLM 不再操心 save, 沉淀全自动.
...
const SAVE_LLM_SYSTEM_PROMPT = [
  ...
  '  "hints": ["bookIds/chapterIds 必须是数组 (复数 s), 单数会 payload-schema-invalid", "返回内容是 markdown 章节全文"],',
