...
src/app/knowledge/
├── controllers/
│   └── knowledge.controller.ts         # REST 控制器 + LLM 知识查询 HookRoute
├── entities/
...
HTTP 路由与 LLM HookRoute 共用同一个 controller。Controller 声明 `@HookController({ pluginName: 'knowledge', tags: ['knowledge'] })`。
Hook 全部通过 `@HookRoute(args)` 声明数组形参 schema (zod, SSOT), 方法签名通过 `z.infer` 复用类型, invoker 在执行前自动校验。
