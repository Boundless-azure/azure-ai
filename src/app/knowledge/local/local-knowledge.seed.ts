...
export const LOCAL_BOOK_RUNNER_HOOK: KnowledgeBookInfo = {
  ...
  name: 'Runner 手册',
  description:
    'Runner 端完整使用手册: 调用约定 (UUID runnerId + 4 段 hookName + single-object payload)、Unit Core 基座能力 (mongo/terminal/file/ast)、Identity & Ability 本地 RBAC + 数据权限、Solution / 数据触点业务 hook、Runner 时区 UTC 硬约束。' +
...
> ⚠️ **payload 形态注意** :: SaaS hook 统一使用单对象形参。
> payload 是单对象: \`{ ...字段 }\`, 所有字段平铺在这一个对象里, 不是数组。

### 调用方式

\`\`\`
call_hook({
  call:    "saas.app.conversation.sendMsg",
  payload: {
...
- 直接 return 文字是无效的，必须通过 \`call_hook({ call: "saas.app.conversation.sendMsg", payload: {...} })\` 才能让用户收到消息。
...
\`\`\`
call_hook({
  call:    "saas.app.conversation.smartTags",
...
\`\`\`
call_hook({
  call:    "saas.app.conversation.smartSearch",
  payload: {
    sessionId: string,
...
\`\`\`
call_hook({
  call:    "saas.app.conversation.smartMessages",
  payload: {
    sessionId: string,
...
\`\`\`
call_hook({
  call:    "saas.app.conversation.webControl",
  payload: {
...
\`\`\`
call_hook({
  call:    "saas.app.conversation.webControlPageinfo",
...
\`\`\`
call_hook({
  call:    "saas.app.conversation.webControlData",
...
SaaS hook 统一通过 hook-controller 声明, \`payload\` **是单对象** (\`call_hook({ call, payload })\`)。所有入参字段都平铺在这一个对象里, 服务端从对象里取字段, 不再是位置数组：

| 形态 | payload 结构 | 适用 |
|------|--------------|------|
| 单参 | \`{ ...字段 }\` | 所有 \`*list\` / \`*create\` / 单 body 写操作 |
| id 单参 | \`{ id: "<id>" }\` | \`*get\` / \`*delete\` |
| id + body | \`{ id: "<id>", ...body }\` | \`*update\` 以及带 path id 的写操作 |

### ⚠️ id 平铺为 payload 对象的顶层 \`id\` 字段, 不要嵌进 body 子对象, 也不要漏掉它

**反例** (把 id 嵌进子对象)

\`\`\`json
{ "body": { "id": "019d145e-a045-77f5-95b0-7d776640759c", "status": "failed" } }
\`\`\`

**正例** (id 与 body 字段一起平铺在同一个对象里)
...
**规则**:
- 单参查询/创建: \`payload: { ...字段 }\`。
- 详情/删除: \`payload: { id: "<id 字符串>" }\`。
- 更新/安装/卸载等 id+body: \`payload: { id: "<id 字符串>", ...body }\` (id 是这个对象的顶层字段, 跟 body 字段平级)。
- 当前登录身份 (principalId/principalType) 由系统在 hook 调用时**自动注入**, **不要**手工写进 payload。
...
\`\`\`
call_hook({ call: "runner.system.hookbus.search", payload: { tag: "component" } })
\`\`\`
...
- 主动发消息 / 历史检索：见对话 Hook 技能手册 (bookId=\`local_conversation_hook_skill\`)。SaaS hook 统一是单对象形参，传 \`{ ...字段 }\`。
...
| Payload | hook 的单对象入参载荷; SaaS hook 通常是 \`{...}\` (字段平铺在一个对象里) | 参数、入参 |
...
2. **安装** :: 调 \`saas.app.solution.install\`，\`id+body\` 形态: \`{ id: "<solutionId>", runnerIds: [...] }\` (id 与 runnerIds 平铺在同一个对象里)。
...
本章覆盖 \`saas.app.timeZone.*\` 系列 hook (toUtc / fromUtc / now / lookupByIp), **payload 统一是单对象形参: 传 \`{ ...字段 }\`**。
...
\`\`\`
call_hook({
  call:    "saas.app.timeZone.fromUtc",
  payload: {
...
\`\`\`
call_hook({
  call:    "saas.app.timeZone.lookupByIp",
...
Runner 是用户自托管的运行时进程, 跟 SaaS 双向 WS 互联。\`runner.*\` hook 都得通过 \`call_hook({ call: "runner.*.*.*", target: "runner", runnerId: "<uuid>", payload: { ... } })\` 派发, **runnerId 必填且必须是 UUID 格式**。
...
\`\`\`json
call_hook({ "call": "saas.app.runner.list", "payload": { "status": "mounted" } })
\`\`\`
...
\`\`\`json
get_hook_tag({ "target": "runner", "runnerId": "<uuid>" })
\`\`\`
返回 tag 频次榜, 按 tag 缩范围:
\`\`\`json
search_hook({ "target": "runner", "runnerId": "<uuid>", "tags": ["mongo"] })
\`\`\`
...
\`\`\`json
get_hook_info({ "target": "runner", "runnerId": "<uuid>", "hookNames": ["runner.unitcore.mongo.find"] })
\`\`\`
...
| 字段 | 形态 | 错误示例 |
|---|---|---|
| \`call\` | **4 段完整名** \`platform.app.module.action\` | ❌ "terminal.exec" (短名) / ❌ "saas.app.hook.search" (编造) |
| \`runnerId\` | **UUID** (来自 saas.app.runner.list 的 items[].id) | ❌ "测试Runner" (alias) / ❌ 任意人类可读字符串 |
| \`payload\` | **单对象** \`{ ... }\` (字段平铺在一个对象里) | ❌ \`[{ ... }]\` (数组) / ❌ \`{ "input": {...}, "args": [...] }\` (旧 envelope) |
| \`target\` | call 以 \`runner.\` 开头时工具自动归一为 \`runner\`, 可省 | — |

例:
\`\`\`json
{
  "call": "runner.unitcore.terminal.exec",
  "target": "runner",
...
\`\`\`json
{
  "call": "runner.unitcore.mongo.insert",
...
\`\`\`json
{
  "call": "runner.unitcore.file.write",
  "target": "runner", "runnerId": "<uuid>",
  "payload": {
...
\`\`\`json
{
  "call": "runner.unitcore.file.patchRange",
  "target": "runner", "runnerId": "<uuid>",
  "payload": {
    "path": "src/main.ts",
    "startLine": 5,
...
\`\`\`json
{
  "call": "runner.unitcore.ast.analyze",
  "target": "runner", "runnerId": "<uuid>",
...
\`\`\`json
search_hook({ "target": "runner", "runnerId": "<uuid>", "pluginName": "unit-core" })
\`\`\`
...
\`\`\`json
{
  "call": "runner.app.solution.search",
  "target": "runner", "runnerId": "<uuid>",
...
\`\`\`json
{
  "call": "runner.app.solution.delete",
  "target": "runner", "runnerId": "<uuid>",
...
/**
 * Code Agent 开发手册 — 二次修改与按 tag 反查
 * @keyword-en code-agent-modify-and-tag-search-chapter
 */
export const LOCAL_CHAPTER_CODE_AGENT_DEVELOPMENT_MODIFY: KnowledgeChapterInfo =
  {
    id: 'local_code_agent_development_handbook_modify',
    bookId: 'local_code_agent_development_handbook',
    title: '二次修改与按 tag 反查',
    sortOrder: 40,
    isLmRequired: false,
    content: `# 二次修改与按 tag 反查关联文件

用户在一个**已存在的 app** 上提"改一下 / 加个东西 / 调整某处"时, 这是**二次修改**, 不是从零新建。核心变化只有两点: (1) 目标是复用既有 app, 不 bootstrap 新建; (2) 规划任务时可以产出 **modify (就地改已存在文件)**, 不再只有 create。

## 先定位, 再改 —— 别凭猜改文件

要改一个文件, 必须先**精确定位**到它现在在哪、现在长什么样。change-plan 节点给了你一条按注释标签反查的快通道:

### 1. read_tags — 先看这个 app 声明了哪些 tag

\`\`\`
read_tags({ routeId })   // routeId = 你要改的那个 target
\`\`\`

返回该 app \`tags.json\` 的**维度化词表** \`{功能职责:[...], 技术栈:[...], 数据接口:[...], 依赖关系:[...], 其他:[...]}\`。这些是这个 app 里**真实声明过**的注释关键词 —— 每个源文件顶部的 \`@keyword\` 注释都会汇总到这里。

### 2. search_by_tag — 用一个已声明的 tag 反查关联文件

\`\`\`
search_by_tag({ tag, routeId })
\`\`\`

内置 ripgrep 快速在该 target 下找出 \`@keyword\` 注释里带这个 \`tag\` 的**所有文件**, 并把每处命中的**整个注释节点**(那段 JSDoc/注释 + 它注释的声明)一并返回。这是定位"要复用/要改的那段代码"最快、最准的方式。
...
定位清楚后, 用 \`upsert_task\` 规划:

- **改已存在的文件** → \`op: "modify"\`, \`path\` 填**该文件真实的现有路径**(就是 search_by_tag / read_file 给你的路径)。生成阶段会读现文件、用 edit_file 定点改, 不会重写整份、不碰无关部分。
- **加新文件** → \`op: "create"\`(默认), 和首次开发一样。

一次二次修改经常是**混合**的: 改几个既有文件(modify) + 加一两个新文件(create)。按每个文件的实际情况分别给 op。

> modify 只用于你**确实定位到**的文件。没定位到、不确定路径, 就先 search_by_tag / search_files / read_file, 别直接 op:modify 一个猜的路径 —— 路径对不上会落空。

## 终端里也能直接用 rg

Runner 终端 (run_terminal / runner.unitcore.terminal.exec) 的 PATH 里已内置 ripgrep, 需要更自由的检索时可以直接 \`rg <pattern> <path>\`。但规划阶段优先用 search_by_tag —— 它按注释语义反查并回注释节点, 比裸 rg 更贴近"找对要改的地方"。
`,
  };
...
初始化由 project-init 节点执行: 它给 LLM 一个 \`run_terminal\` 工具, **命令直接在该 app 目录里跑** (cwd 已经是 app 目录, 所以用 \`.\` 指当前目录, 不要再传 \`<appDir>\`)。终端**没有 stdin**, 命令必须**完全非交互**——任何弹提示/等确认的命令都会挂到超时。**统一用 pnpm** (Runner 配了 workspace 同卷的共享 store, 第二个 app 起装依赖近乎秒过)。按顺序:

1. **脚手架 (非交互, pnpm)**: \`pnpm create astro@latest . --template minimal --install --no-git --skip-houston --yes\`
   - \`.\` = 当前 app 目录; \`--yes\` / \`--skip-houston\` 关掉所有交互提示 (含"目录非空"确认); \`--install\` 顺带用 pnpm 装依赖, \`--no-git\` 不初始化 git。
   - 若某版本 CLI 不认 \`--install\`, 再补一步 \`pnpm install\`。
2. **校验**: 确认 app 目录根出现 \`package.json\`、\`astro.config.mjs\` 和 \`src/\`。有这些即视为初始化完成 (系统也会用 \`package.json\` 存在做兜底校验)。
3. 初始化阶段**只把项目骨架 + 依赖装好, 不写业务源文件** (源文件由后续生成节点填)。

命令要点:
- **一律 pnpm**: 脚手架/装包/构建都用 pnpm (\`pnpm create\` / \`pnpm add\` / \`pnpm remove\` / \`pnpm build\`), 别混 npm/yarn —— 只有 pnpm 才吃共享 store。
- **全程非交互**: 会交互的命令一律加 \`--yes\`/\`-y\`/\`--skip-*\`/\`--no-*\` 关掉; 没有 stdin。
- **幂等**: app 目录可能已存在 (甚至已有 \`tags.json\`), 脚手架靠 \`--yes\` 接受"目录非空"继续, 不要因非空而中止。
- 参考命令 (初始化不需要跑): \`pnpm dev\` 本地开发; \`pnpm build\` → 产物在 \`dist/\`。
...
## 构建验证 (build-test 节点, 生成之后)

代码全部生成后, build-test 节点验证项目**能不能 build**。这一步要**快**——就是"构建一次 + 看结果", 不是排查, 别乱翻文件:

1. **只跑一次** \`pnpm build\` (Astro 站点的构建命令; 产物落 \`dist/\`)。别反复跑。
2. **看这次输出**:
   - **成功** (exit 0、结束时打印 \`Complete!\` / 生成了 \`dist/\` 页面数) → **判构建通过, 一个返修都别提, 直接停**。构建干净时**不要**再去开文件"复查"。
   - **失败** → Astro 的报错**已经指名了出错文件 + 行号 + 原因** (如 \`src/components/Nav.astro:12 - Cannot find ...\` / 未闭合标签 / 找不到导入 / frontmatter 语法错)。通常**光看报错就知道改哪**, 不必再 read_file; 实在看不懂再读那一个文件。
3. 每个真出错的文件提**一条** \`add_repair_task({ path, issue })\`, issue 写**原始报错 + 要修什么** (如 "\`import BaseLayout from '../layouts/BaseLayout.astro'\` 路径错, 实际在 src/layouts/"). 一个坏文件一条, 只能提**已规划文件**。

判定权在你 (不是只看 exit code: 0 退出但页面明显坏也算问题; 有些 warning 可接受)。但**构建干净就相信它、停手**, 别去找茬。常见 Astro build 报错速读: 找不到导入=路径/文件名对不上; 未闭合/多余标签=模板语法; frontmatter (\`---\` 之间) TS 报错=脚本区类型/引用错; 组件 props 类型不符=接口对不上。
`,
};
...
> = new Map([
  ...
  [
    ...
    [
      ...
      LOCAL_CHAPTER_CODE_AGENT_DEVELOPMENT_MODIFY,
    ],
  ],
  [
