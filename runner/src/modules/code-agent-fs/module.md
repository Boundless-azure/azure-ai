...
给 SaaS code-agent 的**并发代码节点**提供 **plan 作用域**的文件读写/搜索能力, 通过 `runner.app.codeAgentFs.{writeTaskFile,writeFile,editFile,readFile,grep,fastSearch…}` 等业务 hook 暴露。核心约束两条:
1. **写分两路** —— `writeTaskFile` 按 `planId+taskId` 从 plan 里取该 task 存的 `path` 落盘 (**path 不由 LLM 提供**, 强制"生成只填已规划的文件, 不即兴造", 要造新文件回 change-plan 加 task); `writeFile` 按 LLM 提供的 path 逐字写 (仅 project-init 写 tags.json 等非 task 文本, path 仍围栏在目标根内)。
...
- `RunnerCodeAgentFsService.writeFile(payload)` — 按 LLM 提供的 workspace 相对 path **逐字**写 plan 目标根内文件 (越界拒、目录自建); 内容走工具入参原样落盘、不经 shell → 供 project-init 写 tags.json 等非 task 文本, 根除手搓文件被逐行转义写坏 | keywords: write-file, scope-fence
...
- `RunnerCodeAgentFsService.readFile(payload)` — 读 plan 目标根内一个文件; **默认不传行号→返回整份 (整份优先: 改前先整份读拿全貌再批量 edit)**, 仅超大文件用可选 1-based startLine/endLine 读宽裕窗口; 回 `{path, content, startLine, totalLines}` | keywords: read-file, whole-file-first
...
- `WriteFilePayloadSchema` / `WriteFilePayload` — writeFile 入参 `{ planId, path, content }` (逐字落盘) | keywords: write-file-payload, verbatim-write
...
- Runner hook: `runner.app.codeAgentFs.writeFile`, payload `{ planId, path, content }`, 返回 `{ path, bytes }`; path 由 LLM 提供但须在 plan 目标根内 (越界拒、目录自建), 内容逐字落盘; 供 project-init 写 tags.json 等非 task 文本; requiredAbility = `create solution`。
...
- Runner hook: `runner.app.codeAgentFs.readFile`, payload `{ planId, path, startLine?, endLine? }`, 返回 `{ path, content, startLine, totalLines }`; path 须在 plan 目标根内; **默认不传行号→整份 (整份优先)**, 仅超大文件给 startLine/endLine 读宽裕窗口。
...
- 写 hook (`writeTaskFile`/`writeFile`) requiredAbility = `create solution`; 读 hook (`readFile`/`grep`/`fastSearch`) = `read solution`。
