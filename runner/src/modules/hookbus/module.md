...
- RunnerHookBusService.previewSchemaValue(value) — 返回完整 payloadSchema 供 LLM 逐字段对齐 payload, 仅极端超长 (>16000) 才截断 | keywords: schema预览, payload校验, schema-preview, payload-validation -> runner_hookbus_schema_preview_019
...
- 校验失败返回 `payload-schema-invalid: <field>: <message>; expectedPayloadSchema={完整 schema}`, 不进入 handler; schema 错误直接按返回的**完整** schema 修 payload (L2 自愈), 不触发 init_tip / hook discovery 补救提示; 同一 hook 第二次仍 schema 失败由 SaaS call_hook 升级到 get_hook_info (L3)
