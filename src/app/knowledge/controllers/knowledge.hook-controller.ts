 ...
 * @description 知识 hook-controller 内直接声明 LLM hook 的单对象形参 schema, type 由 z.infer 派生。
...
/**
 * @title 知识 Hook Controller
 * @description knowledge 模块的 hook 声明层 (单对象 payload); 从 KnowledgeController 迁出, HTTP 与 hook 解耦。
 *   每个 hook 直接调 KnowledgeService, Agent 可见书本范围从 event.context 解析。
 * @keywords-cn 知识Hook声明, 单对象payload
 * @keywords-en knowledge-hook-controller, single-object-payload
 */
@Injectable()
@HookController({ pluginName: 'knowledge', tags: ['knowledge'] })
export class KnowledgeHookController {
