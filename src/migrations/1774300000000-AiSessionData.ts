...
export class AiSessionData1774300000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE chat_sessions_data ADD COLUMN data_key VARCHAR(255) DEFAULT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE chat_sessions_data ADD COLUMN data_title VARCHAR(255) DEFAULT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE chat_sessions_data DROP CHECK chat_sessions_data_data_type_check`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX idx_csd_ai_key ON chat_sessions_data (for_session_id, data_type, data_key)`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP INDEX idx_csd_ai_key ON chat_sessions_data`);
    await queryRunner.query(
      `ALTER TABLE chat_sessions_data DROP CONSTRAINT chat_sessions_data_data_type_check`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE chat_sessions_data DROP COLUMN data_title`,
    );
    await queryRunner.query(
      `ALTER TABLE chat_sessions_data DROP COLUMN data_key`,
    );
  }
}
