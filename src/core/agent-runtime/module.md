...
- `processOneCallAftermath(entry, reply, ctx, sideEffects?)` — per-call 失败 hint 注入 + 副作用回调；payload-schema 错第 2 次失败升级到 get_hook_info/可能非 payload 问题 (L3 保底)，非 schema 错连续 3 次给 init_tip/debug hint | keywords: aftermath, per-call, schema-fail-escalate
