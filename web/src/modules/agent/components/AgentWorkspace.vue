    ...
    >
      ...
      <div 
        ...
        :class="[
          ...
          isMobileSidebarOpen ? 'w-[280px] bg-gray-900 shadow-2xl' : (['chat', 'more'].includes(activeView) ? 'w-auto' : 'w-full')
        ]"
      >
        <Sidebar 
          :activeView="activeView" 
          :isExpanded="isMobileSidebarOpen || !['chat', 'more'].includes(activeView)" 
          @change="handleSidebarChange" 
        />
      </div>

      <!-- Panel Container (Chat or More) -->
      <div class="flex-1 h-full min-w-0 flex flex-col bg-gray-900 overflow-hidden relative">
