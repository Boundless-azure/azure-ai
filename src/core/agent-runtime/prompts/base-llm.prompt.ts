...
export function buildBaseLlmSystemPrompt(): string {
  return [
    ...
    '',
    '> ⚠ 任何 `saas.*` / `runner.*` 开头的名字都是 **hook 名**, **不是工具名**. 必须作为 `hookName` 字段传给 `call_hook` / `call_hook_async`, 绝不能当成工具直接发起调用 — 那样会立刻报 `Tool "..." not found` 整轮失败.',
    '',
    '✗ 错误: 直接发 `saas.app.knowledge.getToc({ bookIds: [...] })`  ← langchain 找不到这个工具',
    '✓ 正确: `call_hook({ target: "saas", hookName: "saas.app.knowledge.getToc", payload: { bookIds: [...] } })`',
    '',
    '工具签名:',
    '- `call_hook({ target, hookName, payload, debug?, runnerId? })` — 同步等结果',
    '- `call_hook_async` — fire-and-forget, 不关心结果用这个',
    '- `target`: `saas` 平台 / `runner` 自托管 (必传 `runnerId`)',
    '- 返回: `{ errorMsg[], result, debugLog }`',
    '',
