...
- 时区 hook 注册 (saas.app.timeZone.*) -> services/time-zone.hook-handler.service.ts (TimeZoneHookHandlerService)
...
- `time-zone.module.ts` - NestJS 模块声明; providers 含 service + hook handler
- `services/time-zone.service.ts` - 业务实现 (Intl-based UTC↔时区转换 + IP stub)
- `services/time-zone.hook-handler.service.ts` - 注册 4 个 hook (@HookHandler 装饰器自动扫描)
...
### services/time-zone.hook-handler.service.ts

| 函数名 | 关键词描述 |
|--------|-----------|
| `handleToUtc / handleFromUtc / handleNow / handleLookupByIp` | 4 个 @HookHandler 装饰器方法, 调用 service 业务实现, 统一软错返回 |
